#!/usr/bin/env bash
# Profile MOAI micro-benchmark: MOAI_BENCH_MODE=gelu -> gelu_test()
# Artifacts default to <repo>/output/gelu/
#
# NVTX: moai:gelu_v2_batch (see test_gelu.cuh). Requires MOAI_HAVE_NVTX build.
#
...
OUT_DIR="${MOAI_PROFILE_DIR:-${REPO_ROOT}/output/gelu}"
OUT_BASE="${MOAI_PROFILE_OUT:-gelu_micro}"
...
for ((i = 1; i <= REPEAT; i++)); do
  ...
  set +e
  (cd "$BUILD_DIR" && export MOAI_BENCH_MODE=gelu && exec nsys profile --force-overwrite=true -o "$stem" --trace="$TRACE" "$TEST_EXE") >"$log" 2>&1
  ec=$?
  set -e
  if [[ $ec -ne 0 ]]; then
  ...
  if [[ "$FILTER_NVTX" == "1" ]]; then
    ...
    nsys stats --report cuda_gpu_kern_sum --force-export=true --format tsv \
      --filter-nvtx "moai:gelu_v2_batch" "$rep" >"$sum_nvtx" 2>"$tmp"
    fc=$?
    set -e
